<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM bermuda WHERE id = '$id'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (isset($_POST['submit'])) {
                ...
                $sql = "UPDATE bermuda SET marca = '$marca', emEstoque = '$emEstoque', emEncomenda = '$emEncomenda',
                        emReserva = '$emReserva'
                        WHERE id = '$id'";
                $result = mysqli_query($conn, $sql);

                if ($result) {
                    echo 'Bermuda atualizada com sucesso.';
                } else {
                    echo 'Erro ao atualizar a Bermuda: ' . mysqli_error($conn);
                }
            }

            // Exibir o formulário de edição
            echo '<h2>Editar Bermuda</h2>';
