<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM moletom WHERE id = '$id'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (isset($_POST['submit'])) {
                ...
                $sql = "UPDATE moletom SET marca = '$marca', emEstoque = '$emEstoque', emEncomenda = '$emEncomenda',
                        emReserva = '$emReserva'
                        WHERE id = '$id'";
                $result = mysqli_query($conn, $sql);

                if ($result) {
                    echo 'Moletom atualizado com sucesso.';
                } else {
                    echo 'Erro ao atualizar moletom: ' . mysqli_error($conn);
                }
            }

            // Exibir o formulário de edição
            echo '<h2>Editar Moletom</h2>';
