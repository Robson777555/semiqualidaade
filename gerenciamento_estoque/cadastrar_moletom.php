<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "INSERT INTO moletom (marca, emEstoque, emEncomenda, emReserva)
                VALUES ('$marca', '$emEstoque', '$emEncomenda', '$emReserva')";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            echo 'Moletom cadastrado com sucesso.';
        } else {
            echo 'Erro ao cadastrar o Moletom: ' . mysqli_error($conn);
        }
    }
    echo '<a href="index.php">Voltar a página inicial</a> ';
?>

<h2>Cadastrar Novo Moletom</h2>
